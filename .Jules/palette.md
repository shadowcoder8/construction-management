...
## 2024-05-04 - [ARIA Labels for Forms]
**Learning:** The app's HTML forms heavily rely on `placeholder` attributes instead of visible `<label>` tags for form inputs and search fields across multiple pages. This makes it difficult for screen readers to properly announce the input's purpose.
**Action:** Always add `aria-label` attributes to input and select elements to provide context for screen readers without altering the visual design or breaking the existing layout.
