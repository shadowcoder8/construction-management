...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        logging.error(f"Login logic error for {login_request.username}: {str(e)}")
        raise e
    except Exception as e:
        logging.error(f"Unexpected login failure for {login_request.username}: {str(e)}")
        raise HTTPException(status_code=401, detail="Invalid username or password")  # Do not leak internal details
