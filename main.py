...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        logging.error(f"Login failed for {login_request.username}: HTTP {he.status_code} - {he.detail}")
        raise he
    except Exception as e:
        logging.error(f"Login failed for {login_request.username}: {str(e)}")
        raise HTTPException(status_code=401, detail="Invalid credentials")  # Generic message to not leak exception details
