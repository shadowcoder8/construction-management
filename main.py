...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        # Re-raise HTTP exceptions (e.g. 500 for missing configuration, 401 for invalid credentials)
        raise he
    except Exception as e:
        logging.error(f"Login failed for {login_request.username}: An unexpected error occurred.")
