...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        # Re-raise HTTPException to preserve specific status codes (e.g., 500 for missing config)
        raise e
    except Exception as e:
        logging.error(f"Unexpected login error for {login_request.username}: {str(e)}")
        raise HTTPException(status_code=401, detail="Invalid username or password")  # Generic unauthorized
