...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        # Re-raise HTTPException to propagate specific status codes like 500 for missing credentials
        raise
    except Exception as e:
        logging.error(f"Login failed for {login_request.username}: {str(e)}")
        raise HTTPException(status_code=401, detail="Unauthorized")  # Use generic unauthorized to avoid exposing info
