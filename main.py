...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        # Re-raise HTTPExceptions (like 500 for missing config or 401 for invalid credentials)
        raise e
    except Exception as e:
        logging.error(f"Login failed for {login_request.username} due to unexpected error.")
        # Return generic error to prevent leaking internal information
