...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        # Re-raise HTTPException to preserve 500 status code for unconfigured credentials
        raise
    except Exception as e:
        logging.error(f"Login failed for {login_request.username}: {str(e)}")
        raise HTTPException(status_code=401, detail="Invalid username or password")  # Unauthorized
