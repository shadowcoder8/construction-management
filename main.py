...
async def admin_login(login_request: models.LoginRequest, response: Response):
    ...
    try:
        ...
        # Re-raise HTTPException to propagate 401 or 500 status codes correctly
        raise
    except Exception as e:
        logging.error(f"Login failed for {login_request.username}: {str(e)}")
        raise HTTPException(status_code=401, detail="Unauthorized")  # Generic error message to avoid leaking internals
